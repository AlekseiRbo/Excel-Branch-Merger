...
def test_failed_new_output_keeps_previous_successful_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    out = tmp_path / "output"
    out.mkdir()
    old_report = out / merger.REPORT_NAME
    old_error = out / merger.ERROR_NAME
    old_log = out / merger.LOG_NAME
    old_report.write_bytes(b"old-report")
    old_error.write_bytes(b"old-error")
    ...
    assert not any(
        path.name.startswith(".excel-branch-merger-") for path in out.iterdir()
    )
