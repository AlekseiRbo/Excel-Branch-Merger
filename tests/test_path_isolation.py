...
def test_own_outputs_and_excel_lock_files_are_not_inputs(tmp_path: Path) -> None:
    ...
    frame = pd.DataFrame({"Customer": ["Alice"], "Date": ["2026-08-01"], "Amount": [100], "Invoice": ["A-1"]})
...
def test_output_folder_is_created(tmp_path: Path) -> None:
    inp = tmp_path / "input"; inp.mkdir(); out = tmp_path / "new-output"
    pd.DataFrame({
    ...
    result = process_folder(inp, out, CONFIG)
    assert out.is_dir()
    assert result.report_path.exists() and result.error_path.exists() and result.log_path.exists()
